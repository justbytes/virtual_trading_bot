...
import {IFPair} from "./IFPair.sol";
import {FRouter} from "./FRouter.sol";
import {FERC20} from "./FERC20.sol";
import {IAgentFactoryV3} from "./IAgentFactoryV3.sol";
...
contract Bonding is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function sell(uint256 amountIn, address tokenAddress) public returns (bool) {
        ...
        uint256 prevPrice =
            duration > 86400 ? tokenInfo[tokenAddress].data.price : tokenInfo[tokenAddress].data.prevPrice;
    ...
    function _openTradingOnUniswap(address tokenAddress) private {
        ...
        address agentToken = IAgentFactoryV3(agentFactory).executeBondingCurveApplication(
            id, _token.data.supply / (10 ** token_.decimals()), tokenBalance / (10 ** token_.decimals()), pairAddress
        );
