...
const main = async () => {
  ...
  const getPair = async (index) => {
    ...
    while (retry < maxRetries) {
      try {
        ...
        retry++;
        console.log(
          `Error fetching pair ${index} (attempt ${retry}/${maxRetries}):`,
          error
        );
        if (retry === maxRetries) {
          throw new Error(
            `Failed to fetch pair ${index} after ${maxRetries} attempts`
          );
        }
  ...
  console.log(
    `Pair addresses saved to pairs.json. Saved: ${validPairs.length} / ${pairsLength}`
  );
};
